...
const ACTIVE_STATUSES = new Set<ActivationJobStatus>(['pending', 'running'])
...
function statusIcon(status: ActivationJobStatus) {
  ...
  if (status === 'skipped') {
    return <SkipForward className='text-muted-foreground size-3.5' />
  }
...
function QueueJobRow(props: QueueJobRowProps) {
  ...
  const active = ACTIVE_STATUSES.has(props.job.status)
  ...
  return (
    <div className='space-y-2 rounded-md border p-2.5'>
      <div className='flex items-start justify-between gap-2'>
        <div className='min-w-0'>
          <div className='flex items-center gap-1.5 text-sm font-medium'>
            ...
            <span className='tabular-nums'>
              {formatTime(props.job.scheduled_at)}
            </span>
        ...
        <Badge
          variant={props.job.status === 'failed' ? 'destructive' : 'outline'}
        >
...
export function ActivationQueueWidget() {
  ...
  if (query.isLoading) {
    ...
    activeJobsContent = activeJobs.map((job) => (
      <QueueJobRow
        key={job.id}
        job={job}
