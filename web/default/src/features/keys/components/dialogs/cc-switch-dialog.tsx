...
function buildCCSwitchURL(
  ...
  const endpoint = normalizedAddress.endsWith('/v1')
    ? normalizedAddress
...
export function CCSwitchDialog(props: Props) {
  ...
  useEffect(() => {
    if (!props.open || modelOptions.length === 0 || models.model) return
    const preferred = modelOptions.some((item) => item.value === 'gpt-5.5')
      ? 'gpt-5.5'
      : modelOptions[0]?.value
    if (preferred) setModels({ model: preferred })
  }, [modelOptions, models.model, props.open])
