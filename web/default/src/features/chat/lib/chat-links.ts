...
function normalizeApiBaseUrl(serverAddress: string): string {
...
export function resolveChatUrl({
  ...
  const safeApiBaseUrl = normalizeApiBaseUrl(safeServerAddress)

  const safeApiKey = normalizeApiKey(apiKey || '')

  if (url.trim().toLowerCase() === 'ccswitch') {
    if (!safeApiKey || !safeApiBaseUrl) return ''
    const params = new URLSearchParams()
    params.set('resource', 'provider')
    params.set('app', 'codex')
    params.set('name', 'QAPI Codex')
    params.set('endpoint', safeApiBaseUrl)
    params.set('apiKey', safeApiKey)
    params.set('model', 'gpt-5.5')
    params.set('homepage', safeServerAddress.replace(/\/+$/, ''))
